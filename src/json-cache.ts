...
const isCacheEntry = (data: unknown): data is CacheEntry<unknown> => {
  return (
    data != null &&
    typeof data === 'object' &&
    'value' in data &&
    'metadata' in data &&
    typeof (data as CacheEntry<unknown>).metadata?.createdTime === 'number'
  );
};
...
export class JsonCache {
  ...
  private async _atomicWrite<T>(
    ...
    try {
      await fs.writeFile(
        tempPath,
        JSON.stringify(entry, null, isDevelopment ? 2 : 0),
        'utf-8'
      );
   ...
   */
  async stats(): Promise<{ size: number; entries: string[] }> {
    try {
      const files = await fs.readdir(this.cacheDir);
 ...
 */
export const createJsonCache = async (
  config: JsonCacheConfig
): Promise<JsonCache> => {
 ...
 */
export const cachified = async <T>(options: {
  cache: JsonCache;
  key: string;
