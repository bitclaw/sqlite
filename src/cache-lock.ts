...
export class CacheLock {
  ...
  constructor(db: Database) {
    ...
    this.deleteExpiredStmt = db.query(
      'DELETE FROM cache_locks WHERE key = $key AND expiration <= $now'
    );
    this.acquireStmt = db.query(
      'INSERT OR IGNORE INTO cache_locks (key, owner, expiration) VALUES ($key, $owner, $expiration)'
    );
    this.checkOwnerStmt = db.query(
      'SELECT owner FROM cache_locks WHERE key = $key'
    );
    this.releaseStmt = db.query(
      'DELETE FROM cache_locks WHERE key = $key AND owner = $owner'
    );
    this.forceReleaseStmt = db.query(
      'DELETE FROM cache_locks WHERE key = $key'
    );
    this.refreshStmt = db.query(
      'UPDATE cache_locks SET expiration = $expiration WHERE key = $key AND owner = $owner'
    );
    this.isLockedStmt = db.query(
      'SELECT 1 FROM cache_locks WHERE key = $key AND expiration > $now LIMIT 1'
    );
    this.pruneExpiredStmt = db.query(
      'DELETE FROM cache_locks WHERE expiration <= $now'
    );
  }
  ...
  acquire(key: string, ttlMs: number, owner?: string): LockResult {
    ...
    const result = this.db.transaction(() => {
      ...
      const row = this.checkOwnerStmt.get({ $key: key }) as {
        owner: string;
      } | null;
